...
def submit_form():
    ...
    mytext = f"""Prepare some lifestyle advice for the prevention of cancer, for a person with the following characteristics: {patient_height}cm tall, weights {weight}kg and is a {age}-year-old {gender}. 
    This person took the following lifestyle and medical history questionnaire and next to each question is the answer obtained. Your esay please separate it into Introduction, Exercise, Sleep, Diet, 
    Communication, Alcohol, Hobbies, Mental Health and Conclusion sections. 
    Please do not give any medical prescriptions in any of the sectons you will provide above."""
    ...
    testtext = "why are my exams so difficult, answer within 15 words"
    ...
    if response.ok:
        ...
        # Save the generated text as an audio file using gTTS
        tts = gTTS(generated_text, lang='en')
        audio_path = 'static/voice.mp3'
        tts.save(audio_path)

        # Render the result template
        return render_template('results.html', generated_text=generated_text, audio_path=audio_path)
    else:
